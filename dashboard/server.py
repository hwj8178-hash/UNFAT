"""
dashboard/server.py — UNFAT Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "UNFAT Uploads",
        Path.home() / "Documents" / "UNFAT Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'UNFAT-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"UNFAT Dashboard Port {port}"
        prog_rule  = "UNFAT Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="unfat_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until UNFAT is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "unfat.key").exists() and (certs / "unfat.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in UNFAT to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('unfat_token','{tok}');
  sessionStorage.setItem('unfat_key','{key}');
  localStorage.setItem('unfat_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to UNFAT…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "unfat.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "unfat.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in UNFAT UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
